...
function createScriptPropertiesStore({ openSpreadsheet, seed }) {
  ...
  /**
   * N'expose QUE les propriétés réellement stockées dans l'onglet. La graine
   * injectée (SPREADSHEET_ID, et sur le chemin du pont DISCORD_BRIDGE_SECRET)
   * reste lisible par `getProperty(clé)` — qui est la seule façon dont Code.gs
   * et DiscordBridge.gs les lisent — mais ne part plus dans un objet complet
   * qu'un appelant pourrait journaliser ou renvoyer tel quel.
   * `_getScriptProperty` (Code.gs) retombe de lui-même sur `getProperty` quand
   * une clé manque à cette carte : aucun comportement ne change.
   */
