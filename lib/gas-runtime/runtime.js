...
const { createRedisLock, lockKeyFor } = require('./redis-lock');
...
const { readDriveVersion } = require('./drive-version');
...
function _buildSandbox({ spreadsheetId, accessToken, scriptId, syncFetch, getDateCtor, bridgeSecret, version, now }) {
  const clock = now || Date.now;
  const driveVersion = version === undefined ? null : version;
  ...
  function openModel() {
    ...
    // Instantané du conteneur si la version Drive n'a pas bougé : 0 lecture Sheets.
    let book = snapshotCache.get(spreadsheetId, driveVersion, clock(), DateCtor);
    if (!book) {
      book = fetchWorkbook({ syncFetch, accessToken, spreadsheetId, DateCtor });
      // Étiqueté avec la version lue AVANT la requête : une modification
      // arrivée entre les deux ne fera que provoquer une relecture de plus.
      snapshotCache.put(spreadsheetId, driveVersion, book, clock());
    }
 ...
 */
/**
 * Verrou d'écriture : Upstash Redis quand il est configuré (0 requête Sheets),
 * sinon la cellule de bail de l'onglet ScriptLock. Tous les points d'entrée
 * doivent recevoir la même configuration, sinon ils ne s'excluent pas.
 */
function _writeLock({ redis, syncFetch, accessToken, spreadsheetId }) {
  if (redis) return createRedisLock({ syncFetch, url: redis.url, token: redis.token, key: lockKeyFor(spreadsheetId) });
  return {
    acquire: t => acquireLock({ syncFetch, accessToken, spreadsheetId, token: t }),
    release: t => releaseLock({ syncFetch, accessToken, spreadsheetId, token: t })
  };
}

function _runInSandbox({ mutating, invoke, spreadsheetId, accessToken, scriptId, syncFetch, readOnly, label, bridgeSecret, idempotencyKey, now, redis }) {
  ...
  const lock = mutating ? _writeLock({ redis, syncFetch, accessToken, spreadsheetId }) : null;
  if (mutating) lock.acquire(lockToken);

  try {
    // Version Drive lue APRÈS le verrou (écriture) : aucun autre écrivain de
    // l'application ne peut la faire bouger avant notre lot.
    const version = readDriveVersion({ syncFetch, accessToken, fileId: spreadsheetId });
    let context = null;
    const env = _buildSandbox({ spreadsheetId, accessToken, scriptId, syncFetch, bridgeSecret, version, now, getDateCtor: () => vm.runInContext('Date', context) });
    ...
    try {
      ...
      if (mutating) {
        ...
        // Le lot est jeté : l'instantané en cache n'a pas été touché, mais la
        // trace d'audit (ou un envoi raté) peut l'avoir rendu faux — on l'oublie.
        snapshotCache.invalidate(spreadsheetId);
        if (trace.length) {
    ...
    if (mutating) {
      ...
      // Même lot, même ordre que ce que Sheets vient d'appliquer : la copie
      // en cache reste identique au classeur. Nouvelle version relue sous le
      // verrou, donc aucun autre écrivain de l'application n'a pu s'intercaler.
      if (requests.length && snapshotCache.applyWrite(spreadsheetId, requests)) {
        snapshotCache.retag(spreadsheetId, readDriveVersion({ syncFetch, accessToken, fileId: spreadsheetId }));
      }
    ...
    if (mutating) lock.release(lockToken);
  }
}

function runApi({ fnName, args, spreadsheetId, accessToken, scriptId, syncFetch, readOnly, idempotencyKey, now, redis }) {
  ...
  try {
    out = _runInSandbox({
      ...
      label: fnName,
      spreadsheetId, accessToken, scriptId, syncFetch: meter.syncFetch, readOnly, idempotencyKey, now, redis,
      invoke: context => {
 ...
 */
function runBridge({ parameter, spreadsheetId, accessToken, scriptId, syncFetch, readOnly, secret, now, redis }) {
  const meter = meterSyncFetch(syncFetch);
  const out = _runInSandbox({
    mutating: true,
    label: 'DiscordBridge:' + String((parameter && parameter.bgAction) || 'inconnu'),
    spreadsheetId, accessToken, scriptId, syncFetch: meter.syncFetch, readOnly, now, redis,
    bridgeSecret: secret,
    invoke: context => {
