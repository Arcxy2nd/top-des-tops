...
const PLAN5_MESSAGE = 'Sauvegardes Drive indisponibles sur ce backend (Vercel) : prévu plus loin dans le Plan 5.';
 ...
 */
function _runInSandbox({ mutating, invoke, spreadsheetId, accessToken, scriptId, syncFetch, readOnly, label }) {
...
/**
 * Pont Discord : DiscordBridge.gs valide lui-même le secret partagé et rend un
 ...
 */
function runBridge({ parameter, spreadsheetId, accessToken, scriptId, syncFetch, readOnly }) {
  const out = _runInSandbox({
    mutating: true,
  ...
  const body = out.value && typeof out.value.getContent === 'function' ? out.value.getContent() : String(out.value);
  return { body: body, appliedWrites: out.appliedWrites };
}

module.exports = { runApi, runBridge, listApiFunctions, API_FUNCTIONS, SCRIPT_TIME_ZONE, PREFETCH_AFTER_MISSES, createUrlFetchApp };
