 ...
 */
function _runInSandbox({ mutating, invoke, spreadsheetId, accessToken, scriptId, syncFetch, readOnly, label, bridgeSecret, idempotencyKey, now }) {
...
function runApi({ fnName, args, spreadsheetId, accessToken, scriptId, syncFetch, readOnly, idempotencyKey, now }) {
  ...
  try {
    out = _runInSandbox({
      ...
      label: fnName,
      spreadsheetId, accessToken, scriptId, syncFetch: meter.syncFetch, readOnly, idempotencyKey, now,
      invoke: context => {
 ...
 */
function runBridge({ parameter, spreadsheetId, accessToken, scriptId, syncFetch, readOnly, secret, now }) {
  const meter = meterSyncFetch(syncFetch);
  const out = _runInSandbox({
    mutating: true,
    label: 'DiscordBridge:' + String((parameter && parameter.bgAction) || 'inconnu'),
    spreadsheetId, accessToken, scriptId, syncFetch: meter.syncFetch, readOnly, now,
    bridgeSecret: secret,
    invoke: context => {
