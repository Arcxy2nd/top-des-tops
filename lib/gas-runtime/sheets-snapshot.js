...
const WORKBOOK_FIELDS = 'properties.timeZone,properties.title,'
  + 'sheets(properties(sheetId,title,gridProperties(rowCount,columnCount)),'
  + 'data.rowData.values(effectiveValue,effectiveFormat.numberFormat.type))';
...
/**
 * Métadonnées ET grilles de tous les onglets en UNE requête : le quota Sheets
 * compte des requêtes, pas des octets. C'est aussi la forme mise en cache
 * entre deux appels (snapshot-cache).
 */
function fetchWorkbook({ syncFetch, accessToken, spreadsheetId, DateCtor }) {
  const url = SHEETS_API + encodeURIComponent(spreadsheetId) + '?includeGridData=true&fields=' + encodeURIComponent(WORKBOOK_FIELDS);
  const json = _getJson(syncFetch, accessToken, url, 'lecture du classeur');
  const grids = {};
  const sheets = (json.sheets || []).map(s => {
    const data = s.data && s.data[0];
    grids[s.properties.title] = gridFromRowData(data && data.rowData, DateCtor);
    return {
      sheetId: s.properties.sheetId,
      title: s.properties.title,
      rowCount: (s.properties.gridProperties && s.properties.gridProperties.rowCount) || 0,
  ...
  return {
    timeZone: (json.properties && json.properties.timeZone) || null,
    // Nom du classeur : cf. fetchSpreadsheetMeta.
    title: (json.properties && json.properties.title) || null,
    sheets,
    grids
  };
...
module.exports = { fetchSpreadsheetMeta, fetchGrids, fetchWorkbook, quoteSheetName, sheetsRequest, sendBatchUpdate, SHEETS_API };
