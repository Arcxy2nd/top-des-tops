...
function createSpreadsheet({ spreadsheetId, sheets, loadGrid, DateCtor, name, copyFile }) {
  const journal = [];
  ...
  function assertAlive(state) {
    if (state.deleted) throw new Error('Cet onglet a été supprimé.');
  }
  ...
  function makeSheet(state) {
    ...
    const sheet = {
      ...
      getLastRow() {
        assertAlive(state);
        return lastRowOf(gridOf(state));
      },
      getLastColumn() {
        assertAlive(state);
        return lastColumnOf(gridOf(state));
      },
      ...
      copyTo(target) {
        ...
        let baseName = 'Copy of ' + state.title;
        let title = baseName;
        let suffix = 2;
        while (states.some(s => s.title === title)) {
          title = baseName + ' ' + suffix;
          suffix++;
        }
        const copy = addSheetState(title, gridOf(state).map(r => (r || []).map(cloneCell)));
  ...
  const spreadsheet = {
    ...
    deleteSheet(sheet) {
      ...
      const deleted = states[index];
      deleted.deleted = true;
      journal.push({ op: 'deleteSheet', sheetId: deleted.sheetId });
      states.splice(index, 1);
    },
    copy(copyName) {
      if (!copyFile) throw new Error('Copie de classeur indisponible dans ce contexte.');
      const created = copyFile(copyName);
      // La copie n'est pas rechargée : Code.gs lit son id, son nom et son URL
      // (BackupService.createSnapshot appelle copy.getUrl()) sur l'objet renvoyé.
      return {
        getId: () => created.id,
        getName: () => created.name,
        getUrl: () => created.webViewLink || ('https://docs.google.com/spreadsheets/d/' + created.id + '/edit')
      };
    }
  };
