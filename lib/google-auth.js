...
// Écriture incluse depuis le Plan 3 : le rejeu du journal poste sur
// :batchUpdate. Le classeur de test doit être partagé en Éditeur avec le
// compte de service, le partage en Lecteur du Plan 1 ne suffit plus.
...
// Indexé par compte de service : un emplacement unique servirait le token d'un
// tenant à un autre dès que deux comptes coexistent dans le même conteneur.
const _tokenCache = new Map(); // client_email -> { accessToken, expiresAt }
...
async function getAccessToken(serviceAccount, fetchImpl) {
  ...
  const cached = _tokenCache.get(serviceAccount.client_email);
  ...
  _tokenCache.set(serviceAccount.client_email, { accessToken: json.access_token, expiresAt: now + (json.expires_in || 3600) });
  return json.access_token;
}

function _resetTokenCacheForTests() { _tokenCache.clear(); }
