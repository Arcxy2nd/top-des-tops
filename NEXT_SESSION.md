...
- **Refonte du sélecteur de période en 2 colonnes ergonomiques (`v3.30.12`)** :
  ...
  - *Colonne droite (~50-55%)* : `.d-period-right-col` (`flex: 52 1 300px; min-width: 270px; padding: 6px 8px;`) réunit en une carte unique le mini-calendrier `.d-cal` occupant toute la largeur (574px, `background: transparent; border: none; padding: 0;`), le texte hint récapitulatif (« 1 jour : du 13 sept. au 13 sept. »), le bloc « Mode de calcul : » avec bordure supérieure subtile, et le récapitulatif live `.d-fill-preview`.
  ...
  - *Tests* : mise à jour de `tests/lot-period.test.js` pour couvrir la disposition 2 colonnes 48/52, l'alignement horizontal strict `nowrap` et l'imbrication (`397/397` tests au vert).
