...
### Modifié
**Humanisé** : Les filigranes d'avatar en arrière-plan ont été entièrement repensés : sur les cartes de notes avec un dégradé doux et un zoom délicat au survol, et sur le Podium avec un traitement héroïque aux reflets métalliques (aura d'or palpitante pour le Champion, halos argent et bronze pour le 2e et 3e). Chaque avatar s'affiche désormais avec un anneau aux couleurs de son joueur.
**Technique** : Modernisation de `.note-card-bg` et `.phrase-podium-bg-avatar` avec masque dégradé horizontal progressif (`mask-image: linear-gradient(to left, black 25%, rgba(0,0,0,0.5) 60%, transparent 95%)`) ; sur le Podium, dimensions élargies (190px pour le 1er, 145px pour le 2e/3e), auras métalliques par rang (`drop-shadow` or, argent, bronze), animation d'aura respirante pour le Champion (`championAvatarAura`), survol dynamique avec montée d'opacité et léger zoom, isolation `z-index` pour garantir la lisibilité du texte et adaptation responsive sur mobile ; anneau coloré (`--player-color`) ajouté aux avatars de colonnes (`.npb-head img`), aux auteurs de notes (`.note-meta-avatar`), aux mentions inline (`.mention img`), aux tuiles Quick Stats (`.qs-avatar`) et au profil actif (`.who-am-i-avatar`) ; prise en charge des parenthèses dans le regex des mentions et masque dégradé harmonisé sur `.notes-flash-avatar-bg`.
...
### Modifié
**Humanisé** : Le bouton « Changer d'utilisateur (se déconnecter) » de la fenêtre de mot de passe s'appelle simplement « Déconnexion ».
**Technique** : `Index.html` — libellé de `#identityPwdChangeUser`.
...
### Modifié
**Humanisé** : Refonte du sélecteur de période de saisie de lot en 2 colonnes ergonomiques : la colonne gauche (~45-50%) regroupe de manière ultra-compacte les onglets de mode, les deux champs de date côte à côte sur la même ligne et les 4 boutons de raccourci sur une seule ligne ; la colonne droite (~50-55%) réunit le calendrier mensuel occupant toute la largeur, le texte récapitulatif, le mode de calcul et le total estimé sans aucun vide vertical.
**Technique** :
- `Index.html` :
  - Restructuration du panneau `.d-period` en 2 colonnes (`align-items: flex-start; gap: 8px;`).
  - Colonne gauche `.d-period-left-col` (~45-50%, `flex: 48 1 280px; max-width: 50%; min-width: 240px; padding: 6px 8px;`) : `.d-period-dates-row` strictement côte à côte (`flex-direction: row; flex-wrap: nowrap; gap: 6px;`), `.d-period-shortcuts` sur une seule ligne (`flex-wrap: nowrap; gap: 3px;`), hauteur compacte de ~121px sans espace vide.
  - Colonne droite `.d-period-right-col` (~50-55%, `flex: 52 1 300px; min-width: 270px; padding: 6px 8px;`) : mini-calendrier `.d-cal` occupant toute la largeur (574px), correction de `min-height: 22px` sur `.d-cal-day`, texte hint récapitulatif, séparateur compact `.d-period-calc-group` et résumé `.d-fill-preview` collés verticalement sans vide.
- `tests/lot-period.test.js` : assertions validant la répartition 48/52 (45-50% gauche, 50-55% droite), l'alignement horizontal strict `nowrap` et l'imbrication du calendrier dans la colonne droite.
