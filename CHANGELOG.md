...
### Corrigé
**Humanisé** : Passe 7 (dernière du registre) sur l'onglet ❓ Guide. Son contenu avait pris du retard sur l'app réelle : la fiche "Outils de données" oubliait deux outils sur cinq (Doublons probables, Mentions manquantes) et trois actions de la carte Santé (rattacher l'auteur des notes, réparer l'ordre, créer un snapshot) ; la fiche Dashboard ne mentionnait pas les boutons "Tout exporter" et "Ce trimestre" ; "Paramètres" annonçait six sous-onglets alors qu'il y en a neuf (Identité et Changelog manquaient) ; l'astuce du Tchat inversait le rythme réel de vérification des messages (4s panneau ouvert, pas fermé) ; "Rapport de santé" parlait d'entrées en double alors qu'il détecte des noms de joueurs/Tops en double. Corrigé aussi : la teinte de survol/encadré du Guide restait figée sur le rouge du thème sombre même en thème clair (variable de couleur jamais déclarée) ; les boutons du menu Guide étaient trop petits au toucher sur mobile (sous la norme déjà en place ailleurs dans l'app) ; les groupes du menu (Onglets principaux / Paramètres / Divers) disparaissaient entièrement sur mobile au lieu de rester lisibles ; la poignée de redimensionnement du menu pouvait écraser le contenu à presque rien sur une fenêtre étroite, ne fonctionnait pas au doigt ni au clavier, et une consultation de note stockée par le navigateur bloquée pouvait empêcher tout clic dans le Guide. Ajouté : une recherche dans le menu du Guide, des renvois internes cliquables ("Voir la section dédiée"), et un support pour lecteur d'écran (le menu se comporte maintenant comme de vrais onglets).
...
### Corrigé
**Humanisé** : Grande passe sur les petits défauts qu'on ne remarque pas mais qui usent à l'usage. Les menus déroulants, le menu d'identité, les bulles d'historique et la liste de suggestions « @Nom » suivent maintenant leur bouton quand la page défile, au lieu de rester figés à l'écran — et se ferment tout seuls quand leur bouton quitte l'écran. La liste de suggestions ne disparaît plus au moindre défilement pendant qu'on tape. L'application ne ralentit plus au fil de l'usage : chaque ouverture d'une fenêtre d'édition laissait derrière elle des restes invisibles qui s'accumulaient jusqu'au rechargement de la page. La touche Échap et le raccourci Ctrl+Entrée fonctionnent désormais sur toutes les fenêtres de l'application, alors qu'ils ne marchaient que sur une poignée d'entre elles — et seulement si le curseur était déjà dans le bon champ. « Confirmer » et « Import en lot », les deux plus utilisées, ne se fermaient pas au clavier du tout. La page ne défile plus derrière une fenêtre ouverte, donc on ne perd plus sa position de lecture. Enfin, la touche Tab ne s'échappe plus d'une fenêtre ouverte pour aller se promener dans la page invisible derrière, et le curseur revient sur le bouton d'origine à la fermeture.
...
### Corrigé
**Humanisé** : La toute première ligne des feuilles Joueurs et Tops était invisible dans l'application — le premier joueur et le premier Top n'apparaissaient nulle part : ni dans les listes, ni sur le tableau de bord, ni dans les filtres. Pire, comme ils étaient invisibles au contrôle des doublons, on pouvait recréer un joueur qui existait déjà, sans aucun avertissement (c'est l'origine du "Ilker" en double). L'application partait du principe que la ligne 1 de chaque feuille contenait des titres de colonnes ; or les feuilles Joueurs, Tops et Historique ont été créées à la main et n'en ont jamais eu — la première vraie donnée était donc prise pour un titre et jetée. Elle regarde maintenant le contenu réel de la ligne 1 avant de décider, sur toutes les feuilles sans exception (Joueurs, Tops, Historique, Notes, Tchat, Barème, Phrases, Journal, Tops alternatifs, Points automatiques, Réglages).
...
### Ajouté
**Humanisé** : Joueurs, Tops, règles du Barème (par Top) et Phrases (par pool) peuvent désormais être réordonnés à la main via deux petits boutons ▲/▼ sur chaque ligne (monter / descendre d'un cran) — identiques à la souris et au doigt sur mobile, sans geste à interpréter. Le nouvel ordre est mémorisé partout : filtres, listes déroulantes de saisie, légendes de graphiques, exports. Un bouton "Réparer l'ordre" dans 🔧 Outils sert de filet de sécurité si la colonne d'ordre venait à manquer ou à être incomplète (par exemple après une modification directe dans Google Sheets) — sans rien réorganiser visuellement, il ne fait que compléter les valeurs manquantes.
