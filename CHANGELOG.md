...
### Corrigé
**Humanisé** : Stabilisation fondamentale du Dashboard mobile : préservation permanente du canevas graphique contre les réécritures du DOM, rétablissement des boutons d'options contextuelles (Totaux, Échelle log, Détaillé par top, Sélecteur Donut) et sécurisation des réponses asynchrones.
**Technique** : `Mobile.html` — (1) `renderDashboardShell()` conserve le squelette HTML existant sans détruire `#mChartWrapper` lors des mises à jour réseau. (2) Ajout des conteneurs `#mChartOptionsBar` et `#mDonutPlayerChips` au DOM mobile. (3) `showChartEmptyState()` et `getOrCreateChartCanvas()` basculent la visibilité du canevas au lieu de le supprimer. (4) Sécurisation de `refreshDashboardData()` avec vérification de la validité de `res.chartData`.
