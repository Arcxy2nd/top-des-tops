...
### Modifié
**Humanisé** : Dans la saisie de lot, chaque ligne n'affiche plus un champ date encombré de raccourcis en permanence : juste une pastille lisible (« 📅 Auj. », « 📅 18 juil. » ou « 📅 12 juil. → 24 juil. » pour une plage). Un clic ouvre un petit éditeur groupé où tout est réuni — raccourcis, date, bascule « Plage de dates », Répéter/Répartir et jours de la semaine. Les lignes sont beaucoup plus nettes, surtout quand il y en a plusieurs.
**Technique** : `Index.html` — la cellule date (`.d-cell`) d'une ligne passe d'un affichage inline permanent à une pastille `.d-pill` + popover `.d-pop` (ouverture au clic, fermeture au clic extérieur/OK/Échap, un seul popover ouvert à la fois). Les champs gardent leurs classes hooks (`.d-start`/`.d-end`/`.range-cb`/`.line-fill`/`.day-picker-wrap`), déplacés dans le popover — aucun changement des sites de lecture (envoi du lot, duplication, « Appliquer à toutes les lignes »). Nouveaux helpers `fmtDateShort()`/`datePillLabel()` + constante `SHORT_MONTHS_FR` ; `applyDateAllBtn` redéclenche un `change` pour rafraîchir la pastille. `Mobile.html` inchangé (saisie de lot mobile à date unique, sans plage — choix assumé).
