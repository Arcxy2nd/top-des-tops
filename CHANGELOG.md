...
**Technique** : `Index.html` — `.nav-btn-label` passe de `display:inline-block` avec transition de `max-width` (qui réservait de la place dans le flux flex, décalant les boutons suivants) à `position:absolute` ancrée sous le bouton (bulle avec petite flèche, `opacity`/`transform` en transition). L'onglet actif garde son libellé affiché en ligne (`position:static`) comme repère permanent.
