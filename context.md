...
- **Apps Script** : outillage de déploiement retiré (workflow, `clasp`, `deploy-targets.json`). Les projets et déploiements Web App Apps Script existent encore côté Google — ils servent le pont Discord tant que BotGhost n'est pas basculé, et de retour arrière — mais ne sont plus mis à jour depuis ce dépôt.
