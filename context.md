...
- **Sonde serveur à la sélection (`unlockOrPrompt`)** : Lors de la sélection d'un joueur marqué protégé par mot de passe dans le cache local, la page effectue d'abord une sonde discrète `apiVerifyIdentity(name, '')` auprès du serveur pour vérifier l'état réel de la feuille sans consigner d'échec dans l'audit. Si le joueur n'a plus de mot de passe, l'accès est accordé instantanément, le cache local est corrigé et le cache serveur des paramètres est invalidé (`_bumpSettingsVersion()`).
