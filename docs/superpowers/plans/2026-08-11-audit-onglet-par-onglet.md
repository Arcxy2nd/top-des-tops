...
**Passes 4 et 5 (Paramètres+Outils, Notes) :**

- **Le garde-fou n°3 peut immobiliser plusieurs passes d'affilée, pas seulement une.** La passe 4 a produit un correctif touchant `apiManageEntity` (doublons de noms) qui attend toujours confirmation utilisateur ; la passe 5 a dû ajouter un second correctif dans la même fonction (`renameEntity` → cascade Notes) avant que le premier n'ait été confirmé/poussé. Les deux se sont empilés dans des commits locaux successifs plutôt que d'être bloqués — solution retenue : continuer à travailler et à committer localement (jamais pousser), documenter clairement l'empilement à chaque étape, et présenter l'ensemble d'un coup à l'utilisateur plutôt que de s'arrêter complètement en attendant une réponse.
- **Le même défaut de fond (`renameEntity`/`deleteEntity` qui ne cascade pas vers toutes les feuilles dérivées) se découvre feuille par feuille plutôt que d'un coup.** Passe 4 a trouvé le trou sur Bareme/Phrases/AutoRules (P8/P9, jamais corrigé — trop risqué en aveugle) ; passe 5 a trouvé le même trou, séparément, sur Notes (N4, corrigé cette fois car le risque était borné — renommage seulement, pas de cascade de suppression). Aurait pu être anticipé en passe 4 en énumérant explicitement *toutes* les feuilles qui référencent un nom de Joueur/Top, plutôt que de découvrir chacune séparément à chaque passe qui la touche.
- **Un conseil à 5 en parallèle peut échouer entièrement sur une limite de session API, sans qu'aucun agent n'ait produit de résultat.** Les 5 agents de la passe 5 ont tous échoué au premier lancement (`session limit`) — relancés à l'identique une fois la limite levée, les 5 ont abouti normalement. Pas une leçon sur le protocole lui-même, mais un rappel opérationnel : un échec de conseil à 5 n'est pas nécessairement un défaut de méthode, vérifier d'abord la cause (quota/session) avant de remettre en cause l'approche.
- **Corriger une classe de bug « globalement » au lieu de « par onglet » devient rentable après 3-4 récidives indépendantes, pas avant.** Le motif `button.small` à 34px a été corrigé de façon scopée en passes 2 et 3 (prudence justifiée : peu de récidives connues à ce stade), puis globalement en passe 4 après une 3ᵉ découverte indépendante. Attention cependant : une règle plus spécifique déjà en place (`.row-topbar .btn-dup`, spécificité CSS supérieure) peut faire échouer silencieusement un correctif global — revérifier après coup que la spécificité CSS ne neutralise pas la règle générale, pas seulement que la règle générale existe.

**Passe 6 (Tchat flottant) :**

- **Un widget hors du système d'onglets peut régresser silencieusement sans qu'aucune passe ne le couvre.** Le Tchat était totalement inaccessible en mobile-layout (aucun bouton, aucun panneau — `display:none !important` inconditionnel hors desktop) depuis la fusion `Mobile.html` → `Index.html` responsive unique (commit `08938f5`), sans qu'aucune des passes 1-5 ne le détecte : les passes précédentes auditaient des onglets du système de tabs, jamais les widgets globaux superposés. À généraliser : tout élément d'interface qui vit hors du flux `#tab-*` mérite un passage explicite, pas seulement une mention en passant.
- **Un défaut de widget peut avoir une cause racine globale, pas locale.** La seconde moitié de R1 (une fenêtre redimensionnée après un chargement étroit reste figée en `mobile-layout`) n'est pas un bug du Tchat mais de `initLayoutModeToggle()` — `applyLayout()` ne s'exécutait qu'une fois au chargement, sans écouteur de redimensionnement. Corrigé par un `matchMedia(...).addEventListener('change', ...)` global (Index.html), qui bénéficie à toute l'app, pas seulement au Tchat — découvert uniquement parce que sa conséquence la plus visible touchait la cible de cette passe.
- **`resize_window` (outil de prévisualisation) ne déclenche pas d'événement `matchMedia`/`resize` natif dans ce pane headless**, même si `window.innerWidth` change bien. Confirmé en isolant un écouteur `matchMedia().addEventListener('change', ...)` fraîchement posé : jamais déclenché après un `resize_window`, alors qu'un rechargement de page à la même largeur détecte correctement la disposition. Limite d'outillage distincte de celle déjà documentée (`document.hidden`/GSAP, passes 2-3) — à anticiper : un correctif basé sur un événement de redimensionnement en direct doit être vérifié par relecture de code + comportement au chargement initial, pas par un redimensionnement live dans ce pane.
